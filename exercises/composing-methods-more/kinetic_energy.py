...
def calculate_kinetic_energy(mass, distance, time):
    if distance.unit != 'km':
        if distance.unit == "light_year":  # [ly] stands for light-year (measure of distance in astronomy)
            # convert from light-year to km unit
            in_km = distance.value * 9.461e12
            new_distance = Distance(in_km, "km")
        else:
...
provided_mass = Mass(2, "solar-mass")
provided_distance = Distance(2, 'light_year')
print(calculate_kinetic_energy(provided_mass, provided_distance, 3600e20))
